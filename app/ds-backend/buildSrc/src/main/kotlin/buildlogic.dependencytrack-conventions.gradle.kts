...
tasks.register("uploadBomToDependencyTrack") {
    ...
    doLast {
        ...
        val envVarName = "DTRACK_${project.name.uppercase().replace('-', '_')}_PROJECT_UUID"
        val projectUuid = System.getenv(envVarName) ?: error("$envVarName environment variable is not set")
        ...
        logger.lifecycle("BOM uploaded to Dependency Track for ${project.name} (HTTP ${response.statusCode()})")
    }
}
